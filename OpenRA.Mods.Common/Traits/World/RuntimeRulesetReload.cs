...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class RuntimeRulesetReload : IChatCommand, IWorldLoaded, IPostWorldLoaded, ITick
	{
		...
		void IPostWorldLoaded.PostWorldLoaded(World w, WorldRenderer wr)
		{
			if (Enabled && w.GameInfo.IsSinglePlayer)
				Game.RulesetWatcher.StartWatching();
		}
		...
		void IChatCommand.InvokeCommand(string name, string arg)
		{
			...
			if (name == CommandName)
			{
				...
				else if (!bool.TryParse(arg, out shouldEnable))
				{
		...
		void ITick.Tick(Actor self)
		{
			...
			if (Game.RulesetWatcher.IsEnabled == isEnabled)
				return;

			if (isEnabled)
				Game.RulesetWatcher.StartWatching();
			else
				Game.RulesetWatcher.StopWatching();
		}
	}
}
